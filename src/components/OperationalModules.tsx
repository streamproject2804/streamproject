...
function RecordForm({title,boilers,close,done}:{title:Exclude<OperationalSection,"Reports">;boilers:Boiler[];close:()=>void;done:()=>void}){
  ...
  const submit=async(e:FormEvent)=>{e.preventDefault();setSaving(true);const user=(await supabase.auth.getUser()).data.user;if(!user){setError("Session expired");setSaving(false);return}
    ...
    if(title==="Shift Handover")payload={shift:form.shift,boiler_id:Number(form.boiler_id),boiler_condition:form.boiler_condition,pending_work:form.pending_work||"",safety_notes:form.safety_notes||"",created_by:user.id};
    else if(title==="Maintenance")payload={boiler_id:Number(form.boiler_id),equipment:form.equipment,work_type:form.work_type,due_date:form.due_date,priority:form.priority,status:"Scheduled",notes:form.notes||"",created_by:user.id};
    else if(title==="Incidents")payload={boiler_id:Number(form.boiler_id),title:form.title,severity:form.severity,description:form.description,corrective_action:form.corrective_action||"",status:"Open",reported_by:user.id};
  ...
  const BoilerSelect=()=> <label>Boiler<select required value={form.boiler_id||""} onChange={e=>update("boiler_id",e.target.value)}><option value="">Select</option>{boilers.map(b=><option key={b.id} value={b.id}>{b.code}</option>)}</select></label>;
  ...
  {title==="Shift Handover"&&<><label>Shift<select required onChange={e=>update("shift",e.target.value)} defaultValue=""><option value="" disabled>Select</option><option>Shift A</option><option>Shift B</option><option>Shift C</option></select></label><BoilerSelect/><label>Boiler condition<textarea required onChange={e=>update("boiler_condition",e.target.value)}/></label><label>Pending work<textarea onChange={e=>update("pending_work",e.target.value)}/></label><label>Safety notes<textarea onChange={e=>update("safety_notes",e.target.value)}/></label></>}
  {title==="Maintenance"&&<><BoilerSelect/><label>Equipment<input required onChange={e=>update("equipment",e.target.value)}/></label><label>Work type<input required onChange={e=>update("work_type",e.target.value)}/></label><label>Due date<input type="date" required onChange={e=>update("due_date",e.target.value)}/></label><label>Priority<select required defaultValue="" onChange={e=>update("priority",e.target.value)}><option value="" disabled>Select</option><option>Low</option><option>Medium</option><option>High</option><option>Critical</option></select></label><label>Notes<textarea onChange={e=>update("notes",e.target.value)}/></label></>}
  {title==="Incidents"&&<><BoilerSelect/><label>Incident title<input required onChange={e=>update("title",e.target.value)}/></label><label>Severity<select required defaultValue="" onChange={e=>update("severity",e.target.value)}><option value="" disabled>Select</option><option>Low</option><option>Medium</option><option>High</option><option>Critical</option></select></label><label>Description<textarea required onChange={e=>update("description",e.target.value)}/></label><label>Corrective action<textarea onChange={e=>update("corrective_action",e.target.value)}/></label></>}
...
function RecordCard({title,row,boilers}:{title:Exclude<OperationalSection,"Reports">;row:RecordRow;boilers:Boiler[]}){
  const boiler=boilers.find(b=>b.id===Number(row.boiler_id))?.code;
