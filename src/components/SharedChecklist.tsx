...
export function SharedChecklist({notify,profile}:{notify:(s:string)=>void;profile:UserProfile|null}){
 ...
 return <section className="module-wrap"><div className="page-heading"><div><p className="eyebrow">SHARED SAFETY INSPECTION</p><h2>Safety checklist</h2><span>{done} of {items.length} checks completed · shared with all users</span></div><div className="completion-badge">{items.length?Math.round(done/items.length*100):0}%</div></div>{error&&<div className="monitor-error">{error}</div>}{profile?.role==="admin"&&<form className="checklist-add panel" onSubmit={add}><input value={text} onChange={e=>setText(e.target.value)} placeholder="Add a new checklist item"/><button className="primary"><Plus/>Add item</button></form>}<div className="checklist-list panel">{items.map((item,i)=><label key={item.id} className={item.is_completed?"checked":""}><button type="button" onClick={()=>void toggle(item)}><CheckCircle2/></button><span><b>{String(i+1).padStart(2,"0")}</b>{item.item_text}</span><em>{item.is_completed?"Completed":"Pending"}</em>{profile?.role==="admin"&&<button type="button" className="check-delete" onClick={()=>void remove(item)}><Trash2/></button>}</label>)}<div className="form-actions"><button className="primary" onClick={()=>notify(`Checklist submitted with ${items.length-done} pending item(s)`)}><ShieldCheck/>Submit checklist</button></div></div></section>;
}
